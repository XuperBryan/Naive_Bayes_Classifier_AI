...
bool NaiveBayes::willDie(int sex, int type, int tubed, int pneumon, int age, int preg, int diabete, int copd, int asthma, int inmsupr, int hyper, int other, int cardio, int obese, int renal, int tobacco, int contact, int covres, int icu){
    ...
    if(contact==1){
        probDie+=2.5*log(dcontact1/dCount);
        probLive+=2.5*log(acontact1/aCount);
    } else if(contact==2){
        probDie+=2.5*log(dcontact2/dCount);
        probLive+=2.5*log(acontact2/aCount);
    } else {
        probDie+=2.5*log(dcontact3/dCount);
        probLive+=2.5*log(acontact3/aCount);
    }
